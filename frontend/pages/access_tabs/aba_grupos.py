...
def aba_grupos(usuario_logado: str):
        ...
        if st.form_submit_button("✅ Criar Grupo", use_container_width=True):
            if not nm_grupo:
                ...
                try:
                    ...
                    existing = supabase.table("tab_app_grupos").select("id_grupo").eq("nm_grupo", nm_grupo).execute()
                    if existing.data:
                        st.error("❌ Este grupo já existe")
                        return
                    
                    # Cria novo grupo
                    supabase.table("tab_app_grupos").insert({
                        "nm_grupo": nm_grupo,
                        "ds_grupo": ds_grupo,
                        "sn_ativo": True
                    }).execute()
        ...
        if grupo_sel:
            ...
            with st.form(f"form_editar_{grupo_sel}"):
                ...
                if st.form_submit_button("💾 Salvar Alterações", use_container_width=True):
                    try:
                        supabase = get_supabase_client()
                        supabase.table("tab_app_grupos").update({
                            "ds_grupo": nova_descricao,
                            "sn_ativo": novo_status
                        }).eq("nm_grupo", grupo_sel).execute()
    ...
    if not df_grupos.empty:
        ...
        if st.button("❌ Deletar Grupo", use_container_width=True):
            try:
                supabase = get_supabase_client()
                supabase.table("tab_app_grupos").delete().eq("nm_grupo", grupo_deletar).execute()
