...
def aba_grupo_pagina(usuario_logado: str):
    ...
    try:
        ...
        resp_paginas = supabase.table("tab_app_paginas").select("*").execute()
        resp_relacoes = supabase.table("tab_app_grupo_pagina").select("*").execute()
    ...
    with st.form("form_permissao"):
        ...
        if st.form_submit_button("✅ Atribuir Permissão", use_container_width=True):
            try:
                ...
                existing = supabase.table("tab_app_grupo_pagina").select("id_grupo_pagina").eq(
                    "id_grupo", id_grupo
                ).eq("id_pagina", id_pagina).execute()
                if existing.data:
                    st.error("❌ Este grupo já tem acesso a esta página")
                    return
                
                # Insere com valores convertidos
                supabase.table("tab_app_grupo_pagina").insert({
                    "id_grupo": id_grupo,
                    "id_pagina": id_pagina,
                    "sn_ativo": sn_ativo
                }).execute()
                
                feedback(f"✅ Permissão atribuída!", "success", "🎉")
                st.rerun()
    ...
    if not df_relacoes.empty:
        ...
        if st.button("❌ Remover Permissão", use_container_width=True):
            try:
                ...
                supabase = get_supabase_client()
                supabase.table("tab_app_grupo_pagina").delete().eq(
                    "id_grupo", id_grupo
                ).eq("id_pagina", id_pagina).execute()
                
                feedback(f"✅ Permissão removida!", "success", "🗑️")
                st.rerun()
