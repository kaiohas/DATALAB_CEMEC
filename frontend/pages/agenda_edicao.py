...
def page_agenda_edicao():
    ...
    try:
        ...
        resp_usuarios = supabase.table("tab_app_usuarios").select("nm_usuario").eq("sn_ativo", True).execute()
        ...
        if selected_rows is not None and len(selected_rows) > 0:
            ...
            if st.session_state.get(f"confirmar_delecao_{agendamento_id}", False):
                ...
                with col_conf:
                    if st.button("✅ CONFIRMAR DELEÇÃO", type="primary", use_container_width=True, key=f"btn_conf_del_{agendamento_id}"):
                        try:
                            # Delete logs primeiro (por causa da constraint)
                            supabase.table("tab_app_log_etapas").delete().eq("agendamento_id", agendamento_id).execute()
                            
                            # Depois delete o agendamento
                            supabase.table("tab_app_agendamentos").delete().eq("id", agendamento_id).execute()
