...
def page_agenda_gestao():
    ...
    try:
        ...
        # =====================================================
        resp_coordenacoes = supabase.table("tab_app_usuario_coordenacao").select("coordenacao").eq("id_usuario", usuario_id).eq("sn_ativo", True).execute()
        ...
        # =====================================================
        resp_status_medico = supabase.table("tab_app_variaveis").select("valor").eq("uso", "status_medico").execute()
        resp_status_enfermagem = supabase.table("tab_app_variaveis").select("valor").eq("uso", "status_enfermagem").execute()
        resp_status_farmacia = supabase.table("tab_app_variaveis").select("valor").eq("uso", "status_farmacia").execute()
        resp_status_espirometria = supabase.table("tab_app_variaveis").select("valor").eq("uso", "status_espirometria").execute()
        resp_status_nutricionista = supabase.table("tab_app_variaveis").select("valor").eq("uso", "status_nutricionista").execute()
        resp_desfecho = supabase.table("tab_app_variaveis").select("valor").eq("uso", "desfecho_atendimento").execute()
        ...
        if selected_rows is not None and len(selected_rows) > 0:
            ...
            with st.form(f"form_status_{agendamento_id}"):
                ...
                if st.form_submit_button("💾 Atualizar Status", use_container_width=True):
                    ...
                    if status_medico:
                        if status_medico_atual != status_medico:
                    ...
                    if status_enfermagem:
                        if status_enfermagem_atual != status_enfermagem:
                    ...
                    if status_farmacia:
                        if status_farmacia_atual != status_farmacia:
                    ...
                    if status_espirometria:
                        if status_espirometria_atual != status_espirometria:
                    ...
                    if status_nutricionista:
                        if status_nutricionista_atual != status_nutricionista:
                    ...
                    if payload:
                        try:
                            ...
                            supabase.table("tab_app_agendamentos").update(payload).eq("id", agendamento_id).execute()
                            
                            # 2️⃣ Insere logs das mudanças
                            for log in logs_para_inserir:
                                supabase.table("tab_app_log_etapas").insert(log).execute()
            ...
            resp_logs = supabase.table("tab_app_log_etapas").select("*").eq("agendamento_id", agendamento_id).order("data_hora_etapa", desc=True).execute()
