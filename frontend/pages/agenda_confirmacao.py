...
def page_agenda_confirmacao():
    ...
    try:
        ...
        resp_status_confirmacao = supabase.table("tab_app_variaveis").select("valor").eq("uso", "status_confirmacao").execute()
        status_confirmacao_list = parse_variaveis(resp_status_confirmacao.data[0]["valor"]) if resp_status_confirmacao.data else []
        ...
        # Busca agendamentos
        resp_agendamentos = supabase.table("tab_app_agendamentos").select("*").order("data_visita", desc=True).limit(500).execute()
        ...
        if selected_rows is not None and len(selected_rows) > 0:
            ...
            with st.form(f"form_confirmar_{agendamento_id}"):
                ...
                if st.form_submit_button("💾 Atualizar Status", use_container_width=True):
                    try:
                        supabase.table("tab_app_agendamentos").update({
                            "status_confirmacao": novo_status
